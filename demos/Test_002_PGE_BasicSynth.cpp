...
public:
    bool OnUserCreate() override
    {
        ...
        engine.SetCallBack_NewSample([this](double dTime)
                                     { return Synthesizer_OnNewCycleRequest(dTime); });
        engine.SetCallBack_SynthFunction([this](uint32_t nChannel, double dTime)
                                         { return Synthesizer_OnGetSample(nChannel, dTime); });
